  ...
  <div>
    <basic-container>
      <avue-crud :option="tableOption"
        ...
        <template slot-scope="scope" slot="menu" v-if="scope.row.status==0" >
          <el-button type="primary"
                     icon="el-icon-edit"
                     size="small"
                     @click.stop="handleEdit(scope.row,scope.index)">通过</el-button>
          <el-button type="danger"
                     icon="el-icon-delete"
                     size="small"
                     @click.stop="handleDel(scope.row,scope.index)" >拒绝</el-button>

        </template>
        <template slot-scope="scope" slot="menu" v-if="scope.row.status==1" >
          <el-button type="primary"
                     icon="el-icon-edit"
                     size="small"
...
<script>
    import {getPageList,add,edit,del} from '@/api/mantuoluo/Tutor/Tutor';
    import { mapGetters } from "vuex";
    import { vTutorOption } from "@/const/mantuoluo/Tutor/TutorTableOption.js";
    export default {
        name: "VTutor",
        components: {},
        data () {
            return {
                tableOption: vTutorOption, //表格设置属性
                tableData: [], //表格的数据
                tableLoading: false,
                tabelObj: {},
                page: {
